...
| Hand-off | **Copy plan** produces the whole plan as text (WhatsApp-ready) **in English or Hindi**, and the page prints as a one-page wall sheet |
...
|---|---|---|
| Engine correctness | **83/83 tests pass** | `npm test` — the same count is rendered in the app badge, regenerated by `scripts/collect_evidence.mjs` |
...
|---|---|
| **Problem & impact (25)** | Ceiling figures cited above; the "same thermometer, different danger" comparison; each output is an *action* (start time, minutes to work, litres, stop hours), not a readout. The hand-off sheet is available **in Hindi**, because a stop-work instruction is worth nothing to a crew that cannot read it. |
...
| **Technical (25)** | Own WBGT engine + standards tables + optimiser in framework-free modules; 83 tests pinning table values, formula properties, the optimiser's ranking and its verdict copy, the shareable URL, the copyable plan text and the demo narrative; external validation of the wet-bulb term. |
| **UX (15)** | One screen: controls → verdict → hour-by-hour timeline → what-if panel → site-conditions → method/evidence. Cited impact strip on the page itself; colour-coded risk; live region for the verdict; `prefers-reduced-motion`; mobile layout; **one-click copy of the plan as text in English or Hindi**, print stylesheet scoped to the crew's plan (the what-if alternatives are hidden in print — on paper they look like the real plan), a **shareable plan URL** (`?city=&cat=&accl=&shift=`) that restores the exact scenario, and an error boundary so a render failure explains itself instead of showing a blank page. |
...
9888c39 15:02  docs: regenerate the provenance trail so every listed hash is reachable from main
