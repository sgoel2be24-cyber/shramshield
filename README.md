...
|---|---|---|
| Engine correctness | **63/63 tests pass** | `npm test` — the same count is rendered in the app badge, regenerated by `scripts/collect_evidence.mjs` |
...
| **Technical (25)** | Own WBGT engine + standards tables + optimiser in framework-free modules; 63 tests pinning table values, formula properties, the optimiser's ranking, the copyable plan text and the demo narrative; external validation of the wet-bulb term. |
| **UX (15)** | One screen: controls → verdict → hour-by-hour timeline → site-conditions panel → method/evidence panel. Colour-coded risk, green highlight for the recommended shift, live region for the verdict, `prefers-reduced-motion` respected, mobile layout, **one-click copy of the plan as text** for handing to a crew, print stylesheet for a site-office wall sheet. |
